...
export class ArtistSearchService {
  ...
  getMyCollection(token: string){
        ...
        return this.http.post('http://www.consigliamiundisco.it/rest/getAlbums', JSON.stringify({ token: token }), options)
                         .map((res:Response) => this.albumMapper(res.json()[0].albums))
                         .catch((error:any) => Observable.throw(error.json().error || 'Server error'));
  }

  private albumMapper(resp: any){
      let albums: Album[] = [];
      for(let albumJSON of resp) {
           let album: Album;
           let tracks: Track[] = null;
           let artista: Artista =  new Artista(albumJSON.artistId,albumJSON.artistName, null, null, null, null, null);
           album = new Album(albumJSON.albumId, albumJSON.albumName, artista, tracks, albumJSON.year, albumJSON.imgUrl );
           albums.push(album);
      }
      return albums;
  }
